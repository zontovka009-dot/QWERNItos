...
async def on_proj_tags(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if ptype == "mods":
        ...
        await update.message.reply_text(
            f"✏️ Шаблон формы для модераторов:\n\n{tpl}\n\n"
...
# ══════════════════════════════════════════════════════════════
#  ШАГ 7 — шаблон формы (только для модераторов)
# ══════════════════════════════════════════════════════════════
...
async def start_create_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    uid   = update.effective_user.id
    cnt   = proj_count(uid)
    limit = proj_limit(uid)

    if cnt >= limit and not is_pro(uid):
        sent = await update.message.reply_text(
            f"📦 У тебя уже {cnt} анкеты — лимит бесплатного тарифа.\n\n"
            "**Pro тариф** даёт безлимит анкет + место в витрине.\n"
            "Начинается от **50 ⭐ в неделю**.",
            reply_markup=K([[B("⭐ Получить Pro", callback_data="pro_menu")]]),
            parse_mode=MD,
        )
