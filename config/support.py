...
async def support_send_callback(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if not text:
        await safe_edit(q, ctx, "Текст потерялся. Попробуй ещё раз.", reply_markup=_main(uid))
        return ConversationHandler.END
...
async def show_support_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
