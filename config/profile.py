...
async def show_profile(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    u   = user_get(uid)
    if not u:
        await safe_edit(q, ctx, "Профиль не найден.")
...
async def show_my_apps(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_edit(q, ctx, text, reply_markup=_back("profile"), parse_mode=MD)
    return ConversationHandler.END
...
async def edit_name_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_edit(q, ctx, "✏️ Напиши имя как хочешь отображаться:", reply_markup=_cancel())
    return S_DNAME
...
async def del_avatar(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_edit(q, ctx, "🗑 Аватар удалён.", reply_markup=_back("profile"))
    return ConversationHandler.END


# ── _msg версия для ReplyKeyboard ────────────────────────────

async def show_profile_msg(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    from config.keyboard import reply_kbd
    uid = update.effective_user.id
    u   = user_get(uid)
    if not u:
        await update.message.reply_text("Профиль не найден.", reply_markup=reply_kbd())
        return ConversationHandler.END
    ...
    if has_av:
        try:
            sent = await update.message.chat.send_photo(
                photo=u["avatar_fid"],
                caption=text,
                reply_markup=K(rows),
                parse_mode=MD,
            ...
            log.warning(f"profile_msg photo: {e}")

    sent = await update.message.reply_text(text, reply_markup=K(rows), parse_mode=MD)
    ctx.user_data["last_bot_msg"] = sent.message_id
    return ConversationHandler.END
