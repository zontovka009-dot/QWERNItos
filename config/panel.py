...
from config.keyboards import safe_edit, delete_prev, reply_main
from config.keyboards import safe_edit, reply_main
...
async def show_tickets(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    ts  = ticket_open()
    if not ts:
        await safe_edit(q, ctx, "Открытых обращений нет.", reply_markup=_back("panel"))
        return ConversationHandler.END
...
async def _render_user_card(q, target_id: int) -> int:
    ...
    if not u:
        await safe_edit(q, ctx, "Пользователь не найден.", reply_markup=_back("pnl_users"))
        return ConversationHandler.END
...
async def show_admin_project(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    p      = proj_get(pid)
    if not p:
        await safe_edit(q, ctx, "Проект не найден."); return ConversationHandler.END
...
async def show_bans(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    bans = ban_list()
    if not bans:
        await safe_edit(q, ctx, "Заблокированных нет.", reply_markup=_back("panel"))
        return ConversationHandler.END
...
async def admin_unban_direct(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    if _guard(q.from_user.id):
        return await _render_user_card(q, target)
    await safe_edit(q, ctx, "✅ Разблокировано.", reply_markup=_main(q.from_user.id))
    return ConversationHandler.END
...
async def admin_unban_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.answer()
    if not _guard(q.from_user.id): return ConversationHandler.END
    await safe_edit(q, ctx, "Отправь Telegram ID для разблокировки:", reply_markup=_cancel())
...
async def admin_search_start(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await q.answer()
    if not _guard(q.from_user.id): return ConversationHandler.END
    await safe_edit(q, ctx, "🔍 Введи Telegram ID, username или имя:", reply_markup=_cancel())
...
async def admin_del_avatar(update: Update, ctx: ContextTypes.DEFAULT_TYPE) -> int:
    ...
    await safe_edit(q, ctx, "🗑 Аватар удалён.", reply_markup=_back(f"adm_user_{target}"))
    return ConversationHandler.END
